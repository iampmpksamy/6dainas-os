...
readonly CASA_EXEC=6dainas-user-service
readonly CASA_SERVICE=6dainas-user-service.service
...
if [[ -z "${CASA_CONF}" ]]; then
    CASA_CONF=/etc/6dainas/user-service.conf
fi

CASA_DB_PATH=$( (grep -i dbpath "${CASA_CONF}" || echo "/var/lib/6dainas/db") | cut -d'=' -sf2 | xargs )
