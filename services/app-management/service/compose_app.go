...
import (
	...
	v1 "github.com/6dainas/6dainas-app-management/service/v1"

	"github.com/6dainas/6dainas-app-management/codegen"
	"github.com/6dainas/6dainas-app-management/common"
	"github.com/6dainas/6dainas-app-management/pkg/config"
	...
	"github.com/6dainas/6dainas-os-Common/utils/port"
	"github.com/6dainas/6dainas-os-Common/utils/random"
		...
		for _, app := range a.Apps() {
			...
			if err != nil {
				if err == ErrComposeExtensionNameXCasaOSNotFound {
					logger.Info("App does not have x-6dainas extension - skipping", zap.String("app", app.Name))
					continue
				}
...
func (a *ComposeApp) SetStoreAppID(storeAppID string) (string, bool) {
	...
	if !ok {
		logger.Info("compose app does not have x-6dainas extension - might not be a compose app for 6DAiNAS-OS", zap.String("app", a.Name))
		return "", false
	}

	composeAppStoreInfo, ok := extension.(map[string]interface{})
	if !ok {
		logger.Info("compose app does not have valid x-6dainas extension - might not be a compose app for 6DAiNAS-OS", zap.String("app", a.Name))
		return "", false
	}
...
func (a *ComposeApp) SetTitle(title, lang string) {
	...
	if !ok {
		logger.Info("compose app does not have valid title map in its x-6dainas extension - might not be a compose app for 6DAiNAS-OS", zap.String("app", a.Name))
		return
	}
...
func NewComposeAppFromYAML(yaml []byte, skipInterpolation, skipValidation bool) (*ComposeApp, error) {
	tmpWorkingDir, err := os.MkdirTemp("", "6dainas-compose-app-*")
	if err != nil {
		return nil, err
	}
