...
readonly CASA_EXEC=6dainas-gateway
readonly CASA_SERVICE=6dainas-gateway.service
...
if [[ -z "${CASA_CONF}" ]]; then
    CASA_CONF=/etc/6dainas/gateway.ini
fi
...
rm -rvf /var/run/6dainas/gateway.pid
rm -rvf /var/run/6dainas/management.url
rm -rvf /var/run/6dainas/routes.json
rm -rvf /var/run/6dainas/static.url
rm -rvf /var/lib/6dainas/www
