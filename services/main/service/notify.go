...
import (
	...
	model2 "github.com/6dainas/6dainas-os/model"
	"github.com/6dainas/6dainas-os/model/notify"
	"github.com/6dainas/6dainas-os/service/model"
...
func (i *notifyServer) SendFileOperateNotify(nowSend bool) {
	if nowSend {
		...
		if len == 0 {
			...
			response, err := MyService.MessageBus().PublishEventWithResponse(context.Background(), common.SERVICENAME, "6dainas:file:operate", msg)
			if err != nil {
				logger.Error("failed to publish event to message bus", zap.Error(err), zap.Any("event", msg))
			}
			if response.StatusCode() != http.StatusOK {
