...
import (
	...
	"github.com/6dainas/6dainas-os-Common/model"
	"github.com/6dainas/6dainas-os-Common/utils/command"
	"github.com/6dainas/6dainas-os-Common/utils/constants"
	"github.com/6dainas/6dainas-os-Common/utils/logger"

	util_http "github.com/6dainas/6dainas-os-Common/utils/http"

	"github.com/6dainas/6dainas-os/common"
	"github.com/6dainas/6dainas-os/pkg/cache"
	"github.com/6dainas/6dainas-os/pkg/config"
	"github.com/6dainas/6dainas-os/pkg/sqlite"
	"github.com/6dainas/6dainas-os/pkg/utils/file"
	"github.com/6dainas/6dainas-os/route"
...
var (
	...
	//go:embed build/sysroot/etc/6dainas/6dainas.conf.sample
	_confSample string
...
func main() {
	...
	urlFilePath := filepath.Join(config.CommonInfo.RuntimePath, "6dainas.url")
	...
	if supported, err := daemon.SdNotify(false, daemon.SdNotifyReady); err != nil {
		logger.Error("Failed to notify systemd that 6dainas main service is ready", zap.Any("error", err))
	} else if supported {
		logger.Info("Notified systemd that 6dainas main service is ready")
	} else {
	...
	logger.Info("6DAiNAS-OS main service is listening...", zap.Any("address", listener.Addr().String()))
